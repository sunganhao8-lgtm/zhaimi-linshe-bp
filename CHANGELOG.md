# 📝 Changelog

本项目所有重要变更的记录。格式基于 [Keep a Changelog](https://keepachangelog.com/zh-CN/1.1.0/)，
版本遵循 [Semantic Versioning](https://semver.org/lang/zh-CN/)。

---

## [Unreleased]

### 🚧 进行中
- 合伙人协作流程文档化
- 移动端布局优化（待办）

---

## [1.1.0] - 2026-06-18

### ✨ 新增：电竞展示区（差异化引流场景）

#### ➕ Added
- **电竞升级为核心差异化**（不再是脚注）：
  - BP 封面加「宠主同乐 · 电竞生活空间」愿景
  - 卖点页加全宽紫色电竞横幅，列为第 1 差异化卖点
  - 品牌定位从"展示型"升级为"核心差异化"，加品牌愿景
  - 文档 tab 定位改为"四位一体（含宠主电竞空间）"
- **风格配色方案**：暖橙（宠物）+ 暗紫（电竞）+ 黑白融合
- bp.md 第 7.5 章节同步 vision + 配色 + 定位升级
- README.md / DECISIONS.md 同步品牌定位

#### 💰 财务影响
- 启动资金保持 **22 万不变**（电竞 0.66 万从流动资金出）
- 月固定成本：**1.57 万/月（不变）**
- 流动资金：3.0 万 → 2.34 万（电竞 0.66 万从中出）
- 回本周期：**24-32 个月（不变）**
- 日均回本 KPI：370 元/天不变

#### 🔧 Changed
- bp.html / bp.md 电竞展示区机位配置修正：投影仪+幕布+Switch 2+沙发+洞洞板（去 PS5/电竞椅/显示器）
- bp.html / bp.md 9.1 表格电竞设备 1.5→0.66 万
- 饼图恢复 5 项（电竞 0.66 万含在流动资金，不单列）
- layout-editor 组件重命名：5 个新组件（Switch 2/投影幕布/电竞沙发/洞洞板/投影仪）
- README.md / DECISIONS.md / CHANGELOG.md 数字同步

#### ⚠️ 假设标注
- 电竞区位置基于"方案 A：转角铺·南门+西墙玻璃"默认假设
- 待用户现场核对后实际调整 layout-editor

---

## [1.0.0] - 2026-06-17

### 🎉 首次发布

#### ✨ 新增
- **首页 `index.html`**：4 大模块入口（文档/演示/门店布局/选址）
- **商业计划书 `bp.html` + `bp.md`**：16 页幻灯片
  - 启动资金 22 万、月固定成本 1.57 万、回本 24-32 个月
  - 49㎡ (7×7) 门店
- **选址地图 `site-map.html`**：
  - 11 个真实 POI（高德地图 API 实测）
  - 真实步行路径 polyline（932m / 12 分钟）
  - POI 点击高亮 + InfoWindow
  - 本地 OSM 瓦片缓存
- **门店布局编辑器 `layout-editor.html`**：
  - 30 个组件（4 外墙 + 2 分区墙 + 3 洗护单间 + 家具设备）
  - 碰撞检测（普通组件 vs 墙 vs 锁组件）
  - 拖动 rAF 节流（移动端流畅）
  - SVG / PNG 导出
- **数据本地化** `data/poi.json`：4427 字节，包含 11 POI + 步行路径
- **品牌资源** `assets/`：Logo、主页背景图
- **GitHub Pages 部署**：在线预览可用

#### 🔧 技术栈
- 高德地图 JS API（离线 loader）
- Konva.js 画布（本地化）
- 纯静态 HTML / CSS / JS，零后端依赖

#### 📚 文档
- `README.md` - 项目说明 + 在线预览链接
- `LICENSE` - MIT 协议
- `CONTRIBUTING.md` - 贡献指南
- `CHANGELOG.md` - 本文件
- `.github/ISSUE_TEMPLATE/` - Issue 模板（bug / feature / 内容更新）
- `.github/workflows/lint.yml` - 自动 HTML/JSON 校验

---

## 版本历史格式说明

- **Added** / **新增**：新功能
- **Changed** / **变更**：已有功能的改动
- **Deprecated** / **弃用**：即将移除的功能
- **Removed** / **移除**：已移除的功能
- **Fixed** / **修复**：Bug 修复
- **Security** / **安全**：安全相关

每条变更需注明作者（如 `@username`）以便追溯贡献。